...
def make_layout() -> Layout:
    ...
    layout["main"].split_row(
        ...
        Layout(name="body", ratio=7, minimum_size=60),
        Layout(name="cover", ratio=2, minimum_size=60),
    )
...
def update_cover_panel(layout, new_content):
    global current_cover_state

    # Only update if the content has changed
    if new_content != current_cover_state:
        layout["cover"].update(make_cover_panel(new_content))
        current_cover_state = new_content  # Update the tracked state
...
update_cover_panel(layout, "Initial Cover Content")  # No update happens
